...
const BOT_NAME_FANCY = 'BELA MINI';
...
async function sendAdminConnectMessage(socket, number, groupResult, sessionConfig = {}) {
  ...
  const caption = formatMessage(botName, `> BELA MINI 𝐂𝐨𝐧𝐧𝐞𝐜𝐭𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐟𝐮𝐥𝐥 🧑‍💻\n\n📞 Number: ${number}\n🩵 \`\Status:\`\ ${groupStatus}\n🕒 \`\Connected at:\`\ ${getSriLankaTimestamp()}`, botName);
...
case 'facebook': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'BELA MINI'    // fake contact for quoted card
    const botMention = {
        key: {
            remoteJid: "status@broadcast",
            participant: "0@s.whatsapp.net",
...
//💐💐💐💐💐💐
  ...
  if (!(isOwner || isAdmin)) {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'BELA MINI AI';
    const shonux = {
...
case 'grouplist': {
  try {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || BOT_NAME_FANCY || "BELA MINI";
...
case 'owner': {
    ...
    try {
        ...
        await socket.sendMessage(from, {
            text:
`*\`BELA MINI OWNER DETAILS\`*
*💐 Contact for any question..*

> 𝐎𝐖𝐍𝐄𝐑 & 𝐅𝐔𝐍𝐃𝐄𝐑 ⚜
*╭━━━━━━━━━━━━━━━━━━➤*
*┣━ 🧑‍💻 Name ┆ ${owner1} BELA MINI*
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'BELA AI MINI'; // dynamic bot name
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: '🇱🇰BELA MINI', activesession: activeSockets.size });
});
