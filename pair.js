...
const config = {
  ...
  FREE_IMAGE: 'https://files.catbox.moe/i4esg2.png',
  NEWSLETTER_JID: '120363420828095666@newsletter', // replace with your own newsletter its the main newsletter
  ...
  SUPPORT_NEWSLETTER: {
    jid: '120363420828095666@newsletter',  // Your channel
  ...
  DEFAULT_NEWSLETTERS: [
    ...
    { 
      jid: '120363420828095666@newsletter', 
      emojis: ['🎵', '🎶', '📻'],
      name: 'Music Updates'
    }
...
const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://thenulathisan53_db_user:s4LJzzHbiYfZH1ms@cluster0.wenhbvp.mongodb.net/?appName=Cluster0'; //we need to create a mongodb url soon
...
╭─「  \`🤖${title}\`  」 ─➤*  
*│
*│*🥷 *Oᴡɴᴇʀ :* ${config.OWNER_NAME || 'ꜰɪxᴏ xᴅ'}
...
case 'creative': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || ' © ʙᴇʟᴀ ᴍɪɴɪ';
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || '© ▶ 𝐁ᴇʟᴀ 𝐌ɪɴɪ 𝐁𝙾𝚃 '}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
